<script lang="ts">
	...
	let isLegalEntity = $state<boolean>(data.settings?.isLegalEntity ?? true);
	...
	const enableLegalEntityIfCompanyLooksIncorporated = (value: string) => {
		if (LEGAL_ENTITY_PATTERN.test(value)) {
			isLegalEntity = true;
		}
	};
</script>
...
>
	...
	<section class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
		...
		<div class="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-4">
			<label class="flex items-start gap-3 text-sm text-gray-700" for="settings-is-legal-entity">
				<input
					id="settings-is-legal-entity"
					name="isLegalEntity"
					type="checkbox"
					bind:checked={isLegalEntity}
					class="mt-0.5 h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
				/>
				<span>
					<span class="block font-medium text-gray-900">Juristische Person</span>
					<span class="text-gray-500">
						Aktiv: `Name` ist Firmenname (BT-27). Inaktiv: `Name` ist Personenname, `Firma` wird als
						Handelsname (BT-28) genutzt.
					</span>
				</span>
			</label>
		</div>
				...
				<div>
					...
					{#if field.name === 'companyName'}
						<input
							...
							oninput={event => {
								enableLegalEntityIfCompanyLooksIncorporated(event.currentTarget.value);
							}}
